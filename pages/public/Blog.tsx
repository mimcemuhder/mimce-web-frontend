          ...
          <div className="absolute bottom-0 left-0 right-0 p-8">
            <div className="flex items-center gap-2 mb-3">
              {(featured.tags ?? []).slice(0, 2).map(tag => (
                <span key={tag} className="text-xs px-3 py-1 bg-white/20 text-white rounded-full backdrop-blur-sm">
                  {tag}
                </span>
              ))}
            </div>
            ...
            <div className="flex items-center gap-3 mt-3 text-white/60 text-xs">
              <span className="flex items-center gap-1"><Calendar size={12} />{new Date(featured.created_at).toLocaleDateString('tr-TR')}</span>
              {featured.author && <span className="flex items-center gap-1"><User size={12} />{featured.author}</span>}
            </div>
          </div>
        </div>
      </Link>
