  ...
  return (
    <>
      <Helmet>
        ...
        <meta
          name="description"
          content="MİMCE mühendislik etkinlikleri — konferanslar, atölyeler ve networking etkinlikleri."
        />
          ...
          {loading ? (
            ...
            <div key={sort} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-7">
              {filtered.map((event) => {
                ...
                const photos = [...new Set(allUrls)].filter(
                  (u) => u && !u.includes('picsum.photos')
                );
                return (
                  <Link
                    key={event.id}
