...
import {
  Bell, Plus, CheckCheck, Trash2, Search, Filter, RefreshCw,
  Info, CheckCircle, AlertTriangle, XCircle, ChevronLeft, ChevronRight,
  Clock, User, ArrowUpDown, SortAsc, SortDesc, X, Send, MailOpen,
} from 'lucide-react';
...
const TYPE_LABELS: Record<TypeFilter, string> = {
  all: 'Tümü', info: 'Bilgi', success: 'Başarı', warning: 'Uyarı', error: 'Hata',
};
...
const NotifTypeIcon: React.FC<{ type: AdminNotification['type']; size?: number }> = ({ type, size = 16 }) => {
...
const Notifications: React.FC = () => {
  ...
  useEffect(() => {
    ...
    return () => { channel.unsubscribe(); realtimeRef.current = false; };
  }, [fetchData]);
  ...
  const handleSearchChange = (v: string) => {
    ...
    searchDebounce.current = setTimeout(() => { setSearch(v); setPage(1); }, 350);
  };
    ...
    <div className="space-y-6">
      ...
      {stats && (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          {[
            { label: 'Toplam', value: stats.total, color: 'bg-gray-50 border-gray-200', textColor: 'text-gray-700', dot: 'bg-gray-400' },
            { label: 'Okunmamış', value: stats.unread, color: 'bg-red-50 border-red-200', textColor: 'text-red-700', dot: 'bg-red-500' },
            { label: 'Bilgi', value: stats.info, color: 'bg-blue-50 border-blue-200', textColor: 'text-blue-700', dot: 'bg-blue-500' },
            { label: 'Başarı', value: stats.success, color: 'bg-green-50 border-green-200', textColor: 'text-green-700', dot: 'bg-green-500' },
            { label: 'Uyarı', value: stats.warning, color: 'bg-yellow-50 border-yellow-200', textColor: 'text-yellow-700', dot: 'bg-yellow-500' },
            { label: 'Hata', value: stats.error, color: 'bg-red-50 border-red-200', textColor: 'text-red-700', dot: 'bg-red-500' },
          ].map(s => (
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
        <div className="flex flex-wrap items-center gap-3">
            ...
            {(['all', 'info', 'success', 'warning', 'error'] as TypeFilter[]).map(t => (
              <button
                ...
                className={`px-3 py-1.5 rounded-md text-xs font-medium transition-all ${
                  typeFilter === t ? 'bg-white shadow-sm text-navy' : 'text-gray-500 hover:text-gray-700'
                }`}
              >
                <span className="flex items-center gap-1">
                  {t !== 'all' && <span className={`w-1.5 h-1.5 rounded-full ${TYPE_DOT[t as AdminNotification['type']]}`} />}
                  {TYPE_LABELS[t]}
                </span>
              </button>
            ))}
          ...
          {/* Okunma filtresi */}
          <div className="flex items-center gap-1 bg-gray-100 rounded-lg p-1">
            {([['all', 'Tümü'], ['unread', 'Okunmamış'], ['read', 'Okunmuş']] as [ReadFilter, string][]).map(([val, label]) => (
              <button key={val} onClick={() => { setReadFilter(val); setPage(1); }}
          ...
          <div className="flex items-center gap-2">
            {selected.size > 0 ? (
              ...
              <>
                ...
                <button onClick={handleDeleteAllRead} disabled={bulkLoading}
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="py-16 flex flex-col items-center gap-4 text-gray-400">
            ...
            {(search || typeFilter !== 'all' || readFilter !== 'all') && (
              <button onClick={() => { setSearch(''); setSearchInput(''); setTypeFilter('all'); setReadFilter('all'); }}
          ...
          <div className="divide-y divide-gray-100">
            {notifications.map(n => (
              ...
              >
                ...
                {/* İkon */}
                <div className={`mt-0.5 w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${TYPE_COLORS[n.type]}`}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <p className={`text-sm text-gray-900 truncate ${!n.is_read ? 'font-bold' : 'font-medium'}`}>
                      {n.title}
                    </p>
                    {!n.is_read && (
                ...
                <div className="hidden sm:flex w-20 justify-center shrink-0">
                  <span className={`px-2 py-1 rounded-full text-[11px] font-semibold border ${TYPE_COLORS[n.type]}`}>
                    {TYPE_LABELS[n.type]}
                  </span>
                </div>
                ...
                <div className="hidden lg:block w-36 text-xs text-gray-400 shrink-0">
                  {new Date(n.created_at).toLocaleDateString('tr-TR', { day: '2-digit', month: 'long', year: 'numeric' })}
                  <br />
                  {new Date(n.created_at).toLocaleTimeString('tr-TR', { hour: '2-digit', minute: '2-digit' })}
                </div>

                {/* Aksiyon butonları */}
                <div className="flex items-center gap-1 shrink-0 w-16 justify-end" onClick={e => e.stopPropagation()}>
                  {!n.is_read && (
                    <button
