  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingCertificateId) {
      ...
      if (!error) {
        setData(
          data.map((c) =>
            c.id === editingCertificateId ? ({ ...c, ...updatedCertificate } as Certificate) : c
          )
        );
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Sertifikaları görüntüleyin, oluşturun ve yönetin.
          </p>
        </div>
        <button
          onClick={() => {
          ...
          <div className="p-4 border-b border-gray-200 flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div className="flex gap-2">
              {['Tümü', 'Yapısal', 'Elektrik', 'Yazılım', 'Güvenlik'].map((cat) => (
                <button
                  key={cat}
                  className="px-3 py-1.5 text-xs font-medium rounded-full border border-gray-200 text-gray-600 hover:bg-gray-50"
                >
                  {cat}
                </button>
              ))}
